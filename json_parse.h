...
static c_json *parse_object(mem_arena *arena, char **cursor) {
  ...
  c_json *node = PUSH_STRUCT_ZERO(arena, c_json);
...
static c_json *parse_value(mem_arena *arena, char **cursor) {
  ...
  if (c == '{') {
    return parse_object(arena, cursor);
  } else if (c == '[') {
    return parse_array(arena, cursor);
  }
...
static inline b32 json_is_number(c_json *n) {
  return n && n->type == TYPE_NUMBER;
}
static inline b32 json_is_string(c_json *n) {
  return n && n->type == TYPE_STRING;
}
static inline b32 json_is_object(c_json *n) {
  return n && n->type == TYPE_OBJECT;
}
static inline b32 json_is_array(c_json *n) {
  return n && n->type == TYPE_ARRAY;
}

static inline double json_get_number(c_json *n) {
  ASSERT(json_is_number(n));
  return n->valuenumber;
}
static inline char *json_get_string(c_json *n) {
  ASSERT(json_is_string(n));
  return n->valuestring;
}
static inline b32 json_get_bool(c_json *n) {
  ASSERT(json_is_bool(n));
  return n->valuebool;
}

static inline double json_get_number_default(c_json *n, double def) {
  return json_is_number(n) ? n->valuenumber : def;
}
static inline char *json_get_string_default(c_json *n, char *def) {
  return json_is_string(n) ? n->valuestring : def;
}
static inline b32 json_get_bool_default(c_json *n, b32 def) {
  return json_is_bool(n) ? n->valuebool : def;
}
