...
static inline void* arena_push(mem_arena *arena, u64 size, b32 clear);
  ...
  static inline s8 str8_substr(s8 str, u64 start, u64 end) {
      ...
      start = MIN(start, end);
      return (s8){.str = str.str + start, .size = end - start};
  }

  static inline s8 str8_append(mem_arena* arena, s8 a, s8 b) {
      s8 out;
      out.size = a.size + b.size;
      out.str = (u8*)arena_push(arena, out.size, false);

      if (out.str) {
          memcpy(out.str, a.str, a.size);
          memcpy(out.str + a.size, b.str, b.size);
      }
      return out;
  }
...
static inline void arena_clear(mem_arena *arena) { arena_pop_to(arena, AREANA_BASE_POS); }
